...
impl<F: GeneralBooleanFunction> OneHotRegisterGame<F> {
    ...
    pub fn create_subgame(&self, ignored: &F) -> symbolic::Result<Self> {
        ...
        Ok(Self {
            ...
            e_i: self
                .e_i
                .iter()
                .flat_map(|e_i| Ok::<_, symbolic::BddError>(e_i.diff(ignored)?.diff(&ignored_edge)?))
                .collect(),
    ...
    fn attractor_set_controller(&self, starting_set: &F) -> symbolic::Result<F> {
        ...
        loop {
            ...
            let e_i_attracted_set = self.e_i_all.and(&edge_starting_set)?.exist(&self.conjugated_v_edges)?;
...
pub mod further_investigation {
    ...
    impl OneHotRegisterGame<BDD> {
        ...
        pub fn attractor_set_early_stop(&self, player: Owner, starting_set: &BDD) -> symbolic::Result<BDD> {
            ...
            loop {
                let edge_starting_set = self.edge_substitute(&output)?;
                let e_i_attracted_vertices =
                    self.small_attractor(&self.e_i_all, &edge_i_set, opponent_set, &edge_starting_set)?;
                ...
                let e_move_attracted_vertices =
                    self.small_attractor(&self.e_move, &edge_move_set, opponent_set, &e_i_starting_set)?;
        ...
        fn small_attractor(
            ...
            let any_edge_set = edge_player_set.and(targets)?.exist(&self.conjugated_v_edges)?;
...
pub struct OneHotEncoder<T, F> {
    /// Map from a unique `T` to the index of the one-hot encoding bit
    pub mapping: HashMap<T, F>,
    variables: EcoVec<F>,
}

impl<T, F> SymbolicEncoder<T, F> for OneHotEncoder<T, F>
    where
        T: Eq + Hash,
        F: Function + BooleanFunctionExtensions {
    fn encode(&mut self, value: T) -> symbolic::Result<&F> {
        self.mapping
            .get(&value)
            .ok_or_else(|| symbolic::BddError::NoMatchingInput)
    }
}

impl<T, F> OneHotEncoder<T, F>
where
    T: Eq + Hash,
    F: Function + BooleanFunctionExtensions
{
    /// Create a new one-hot encoder (technically not an encoder).
    ///
    /// The unique keys are expected to be ordered to match the variables, e.g., unique key `0` will cause variable `0` to be set to `1`.
    pub fn new(unique_keys: impl IntoIterator<Item = T>, variables: EcoVec<F>) -> symbolic::Result<Self> {
        // Pre-compute all possible mappings
        let mut mapping = HashMap::default();
        for (bit_index, key) in unique_keys.into_iter().enumerate() {
            let bit_set = variables[bit_index].clone();
            let one_hot = variables
                .iter()
                .take(bit_index)
                .chain(variables.iter().skip(bit_index + 1))
                .try_fold(bit_set, |acc, other_parts| {
                    Ok::<_, OutOfMemory>(acc.and(&other_parts.not()?)?)
                })?;

            mapping.insert(key, one_hot);
        }

        Ok(Self { mapping, variables })
    }
}
...
3 1 1 2 "3";"#;
    ...
    struct SymbolicTest {
        pg: OneHotRegisterGame<BDD>,
        original: ParityGame,
    }

    fn symbolic_rg(pg: ParityGame, k: Rank) -> eyre::Result<SymbolicTest> {
        let s_pg = super::OneHotRegisterGame::from_symbolic(&pg, k, Owner::Even)?;

        Ok(SymbolicTest { pg: s_pg, original: pg })
    }
}
